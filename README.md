...
A blazing‑fast, minimal‑footprint Global Server Load Balancing (GSLB) client library for Python, powered by a native Rust engine. The current release is **1.2.0** and the package supports CPython **3.9 through 3.12**.
...
The repository contains three language bindings:

* **Python** – the `python/` subproject produces binary wheels via maturin.
* **Node.js** – the `js-node/` crate builds a NAPI native module and is
  packaged as an npm tarball (`.tgz`).
* **Browser** – the `js-browser/` crate compiles to WebAssembly using
  `wasm-pack` and publishes a small `pkg/` directory.

All of the above artifacts are generated by the unified CI workflow and
attached to each GitHub release under the corresponding tag.

### Python

Pre‑built wheels for supported platforms and Python versions are available
on the release page; filenames look like
...
Or after the project is published to PyPI you can simply:

```bash
pip install gslb_rust
```

### Node.js

To consume the Node binding, download the `gslb-node-*.tgz` artifact from a
release and install it into your project directory:

```bash
npm install /path/to/gslb-node-1.2.0.tgz
``` 

Once the package is installed you can `require('gslb-node')` like any other
native module.  (The NAPI binding currently supports Node >= 14.)

### Browser / WebAssembly

The `browser-wasm` artifact directory contains a precompiled WebAssembly
bundle and JavaScript glue code that can be dropped directly into a web
app or bundler.
...
<script type="module">
  ...
  await init();
  const resolver = new GslbResolver([...]);
</script>
```

Alternatively, run one of the `wasm-pack` targets yourself from the
`js-browser` directory.

> The release workflow automatically collects all three artifact types so
you don't have to manage them manually.
